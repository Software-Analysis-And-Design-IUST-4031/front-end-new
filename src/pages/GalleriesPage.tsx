...
//   const theme = useTheme();
  ...
  const handleClickGallery = (userId: number) => {
    if (userId) {
      navigate(`/profile/${userId}`);
    } else {
      console.error('No owner ID provided for gallery');
    }
  };
  ...
  useEffect(() => {
    const fetchGalleries = async () => {
      try {
        setLoading(true);
        const response = await axios.get<GalleryInterface[]>('https://zaferuni.liara.run/api/galleries');
        const baseURL = 'https://zaferuni.liara.run';
        
        const formattedData = response.data.map((gallery: GalleryInterface) => ({
          ...gallery,
                ...
                {currentData.map((gallery, index) => (
                  <Grid item key={index}>
                    <Gallery
                      ...
                      index={index}
                      onclick_gallery={() => gallery.owner_id ? handleClickGallery(gallery.owner_id) : null}
                    />
                  </Grid>
                ))}
              </Suspense>
